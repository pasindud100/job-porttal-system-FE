...
const CompanyPage = () => {
  ...
  return (
    <Box className="bg-green-100 min-h-screen p-6">
      <Navbar />
      <Box className="w-3/4 mx-auto p-6 bg-white rounded-lg shadow-lg mt-5">
        <Button variant="contained" color="success" startIcon={<AddIcon />} onClick={() => setOpenCreateModal(true)}>
          Create Company
        </Button>
        <Box className="mt-4">
          {companies.map((company) => (
            <Box key={company.id} className="border border-green-300 p-4 mb-2 rounded-lg bg-green-50">
              <Typography variant="h6" className="font-bold">{company.name}</Typography>
              <Typography>{`Location: ${company.location}`}</Typography>
              <Box className="flex justify-end mt-2">
                <Button onClick={() => { setUpdateCompanyData(company); setOpenUpdateModal(true); }} variant="contained" color="success" className="mr-2">Update</Button>
        ...
        <Dialog open={openCreateModal} onClose={() => setOpenCreateModal(false)}>
          ...
          <DialogActions>
            ...
            <Button onClick={handleCreateCompany} variant="contained" color="success">Create</Button>
          </DialogActions>
        </Dialog>
        ...
        <Dialog open={openUpdateModal} onClose={() => setOpenUpdateModal(false)}>
          ...
          <DialogActions>
            ...
            <Button onClick={handleUpdateCompany} variant="contained" color="success">Update</Button>
          </DialogActions>
        </Dialog>
      </Box>
    </Box>
